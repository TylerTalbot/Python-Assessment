...
PRODUCTS = {
    ...
    "Forks", "Knives", "Spoons",
    #Audio/Visual
    "Speakers", "Projectors", "Microphones",
    #Entertainment
    "Bouncy Castles", "Clowns", "Magicians"
}
...
Name_Entry = tk.Entry(root, bg="white", width=18)
...
Last_Name_Entry = tk.Entry(root, bg="white", width=18)
...
Item_Entry = tk.Entry(root, bg="white", width=18)
...
Item_Dropdown = ttk.Combobox(root, values=list(PRODUCTS), state="readonly", width=15)
...
def submission():
    ...
    Customer.append({"Receipt": Receipt, "FirstName": First_Name, "LastName": Last_Name, "Item": Item, "Quantity": Quantity})
    Tree_View.insert("", tk.END, values=(First_Name, Last_Name, Item, Quantity, Receipt))
...
def Delet_An_Item():
    ...
    for item in selected:
        ...
        global Customer 
        Customer = [c for c in Customer if c ["Receipt"] != remove]
...
def download_Receipt():
    ...
    with open("Receipt.txt", "a") as file:
        file.write("---Party Supplies Receipt---\n\n")
        for item in Amount:
            ...
            file.write(f"Your recipt number is {Receipt}\n")
            file.write("-------------------------------------\n")
    messagebox.showinfo("Saved", "Your Receipt was saved succesfully")
    return

#------------------------------------------------------------------------------------------------------------------------------------------------------
# This is the treeview where, when the code is running it displays the users inputs
Tree_View = ttk.Treeview(root, column=("First Name", "Last Name", "Item", "Quantity", "Receipt"), show="headings", height=6)
for col, text in [("First Name", "First Name".capitalize()), ("Last Name", "Last Name".capitalize()), ("Item", "Item".capitalize()), ("Quantity", "Quantity"), ("Receipt", "Receipt")]:
...
Submit = tk.Button(root, text="Submit Order", bg="skyblue", font=("sans-serif", 12, "bold"), width=10, command=submission)
Submit.grid(row=7, column=1, columnspan=2, pady=5)
Receipt = tk.Button(root, text="Download Receipt", bg="skyblue", font=("sans-serif", 9, "bold"), width=15, command=download_Receipt)
Receipt.grid(row=7, column=0, columnspan=2, pady=5)
Delete = tk.Button(root, text="Return Your Items", bg="skyblue", font=("sans-serif", 9, "bold"), width=15, command=Delet_An_Item)
Delete.grid(row=7, column=2, columnspan=2, pady=5)
